...
from app.bootstrap import ServiceManager, YoloUdpReceiver
...
def test_yolo_receiver_drops_bad_fields_and_keeps_listening() -> None:
    ...
    try:
        ...
        client.sendto(json.dumps({"schema_version": 1, "sequence": 1,
                                  "captured_at_monotonic": now_mono,
                                  "published_at_monotonic": now_mono,
                                  "target": {"track_id": "invalid"}, "scene": {}}).encode("utf-8"), address)
        client.sendto(
            json.dumps({"schema_version": 1, "sequence": 2,
                        "captured_at_monotonic": now_mono,
                        "published_at_monotonic": now_mono,
                        "target": {"target_valid": True, "tracking_state": "locked", "track_id": 7},
                        "scene": {}}).encode("utf-8"),
            address,
        )
...
def test_service_manager_reconnect_stops_old_link_and_starts_new_link(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.bootstrap.LinkManager", FakeLinkManager)
    old_config = object()
    new_config = object()
