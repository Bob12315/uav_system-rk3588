...
def test_action_lab_only_loop_does_not_deadlock():
    ...
    assert isinstance(snapshot, dict), "latest_snapshot should be a dict"
    assert "field_reference" in snapshot, (
        "latest_snapshot missing 'field_reference' key"
    )


# ---------------------------------------------------------------------------
# Test 2: field_reference_controller.status() must NOT be called inside
#         the control_command_log_lock
# ---------------------------------------------------------------------------


class _LockDetectingController:
    """Wraps FieldReferenceController to detect lock-held calls."""
    ...
    def status(self):
        if self._lock.locked():
            raise AssertionError(
                "field_reference_controller.status() called while "
...
def test_status_not_called_inside_lock():
    """field_reference_controller.status() must never be called while
...
def test_websocket_handler_uses_to_thread():
    ...
    server_path = (
        Path(__file__).resolve().parents[2] / "web_ui" / "server.py"
    )
