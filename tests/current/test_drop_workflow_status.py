...
def test_system_runner_has_drop_workflow_helpers():
    """Verify the required helper methods exist on SystemRunner."""
    from app.system_runner import SystemRunner
    assert hasattr(SystemRunner, "_workflow_targets")
    assert hasattr(SystemRunner, "_rank_by_target_id")
    assert hasattr(SystemRunner, "_rank_by_target_xy")
    assert hasattr(SystemRunner, "_drop_rank_from_result")
    assert hasattr(SystemRunner, "_save_drop_workflow_from_action_result")
    assert hasattr(SystemRunner, "_ensure_drop_workflow")
...
def test_drop_rank_from_result_by_id():
    ...
    wf = runner._ensure_drop_workflow()
    wf["selected_targets"] = [
    ...
    rank = runner._drop_rank_from_result("target_lock", detail)
    assert rank == 2

    # target_lock with detail.target containing target_id
    detail2 = {"target": {"target_id": "obj_a"}}
    rank2 = runner._drop_rank_from_result("target_lock", detail2)
    assert rank2 == 1
...
def test_drop_rank_from_result_by_key():
    ...
    rank = runner._drop_rank_from_result("payload_release", detail)
    assert rank == 2
...
def test_select_drop_targets_builds_canonical():
    ...
    runner._save_drop_workflow_from_action_result("select_drop_targets", result)
    wf = runner._ensure_drop_workflow()
...
def test_target_lock_then_release_preserves_states():
    ...
    runner._save_drop_workflow_from_action_result("select_drop_targets", {
        "done": True,
        "detail": {
            "selected_targets": [
                {"id": "t1", "x": 1.0, "y": 2.0},
    ...
    # target_lock for t1
    runner._save_drop_workflow_from_action_result("target_lock", {
        "done": True,
    ...
    runner._save_drop_workflow_from_action_result("payload_release", {
        "done": True,
    ...
    runner._save_drop_workflow_from_action_result("land", {"done": True, "detail": {}})
    wf = runner._ensure_drop_workflow()
    assert wf["selected_targets"][0]["status"] == "released"
    assert wf["selected_targets"][1]["status"] == "locked"
