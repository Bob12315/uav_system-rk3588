...
class GpsReconSequenceAction(GpsTargetSequenceCore, ActionModule):
    ...
    def _transition_after_operation(self, target, ctx, actions):
        ...
        self.sub_action = None
        return ActionResult(
            actions=list(actions or []) + self._stop_actions("sequence_done"),
            done=True,
    ...
    def _update_operation_hook(self, ctx):
        ...
        if hover.failed:
            return ActionResult(failed=True, reason="gps_recon_hover_failed", actions=self._stop_actions("hover_failed"), detail=self._sequence_detail())
        ...
        if elapsed_s < self.observe_duration_s:
            return ActionResult(actions=hover.actions, reason="gps_recon_observing", detail=self._sequence_detail())
        ...
        return ActionResult(done=True, reason="gps_recon_observing", actions=hover.actions, detail=self._sequence_detail())
