...
class GpsTargetSequenceCore:
    ...
    def reset(self):
        self.targets=[]; self.phase="idle"; self.target_index=0; self.sub_action=None; self.update_count_at_phase=0; self.started=False; self.stopped=False; self._failed_reason=""; self._climb_target_lat=None; self._climb_target_lon=None; self.phase_history=[]
    ...
    def _update_align(self,ctx):
        ...
        if not r.done and (not isinstance(command,dict) or not command.get('valid') or not command.get('active')): return ActionResult(actions=self._stop_actions('align_inactive'),reason=self._sequence_reason('align_inactive'),detail=self._sequence_detail(extra={'align':r.detail}))
    ...
    def _update_operation(self,ctx):
        ...
        if r.failed:return self._fail('operation_failed',r.actions)
    ...
    def _clear_continuous_command(self,key): return {'action_type':'clear_continuous_commands','params':{'clear_pending_local_position':False,'send_stop_first':True},'once':True,'key':f'gps_drop_clear_{key}'}
