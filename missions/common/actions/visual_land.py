...
class VisualLandAction(ActionModule):
    ...
    def update(self, context: dict[str, Any] | None = None) -> ActionResult:
        ...
        if self.done:
            return ActionResult(actions=[], done=True, reason="visual_land_done",
                                detail=self._last_align_result.detail if self._last_align_result else {})
        if self.failed:
            return ActionResult(actions=[], failed=True, reason=self.failure_reason)
