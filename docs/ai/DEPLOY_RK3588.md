...
4. [`../developer/safety.md`](../developer/safety.md)；
5. 本文。
...
首次启动前应提醒用户拆除螺旋桨和正式载荷。不得输出、记录或提交密码、SSH 私钥、
Token、Wi-Fi 密码等秘密信息。
...
脚本会检查 Linux ARM64、环境名和 Python 版本，并根据仓库当前 requirements 安装依赖。
...
```bash
python -c "import cv2, numpy, yaml; from rknnlite.api import RKNNLite; print(RKNNLite)"
