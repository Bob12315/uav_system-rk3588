...
from execution.safety_config import SafetyConfig, load_safety_config
...
class ContinuousCommandGuard:
    ...
    def _invalid_reason(self, active: dict[str, object]) -> str | None:
        ...
        state_port = active["state_port"]
        get_source = getattr(state_port, "get_active_source", None)
        if callable(get_source) and get_source() != active["source"]:
            return "continuous_source_changed"
        get_state = getattr(state_port, "get_latest_drone_state", None)
    ...
    def _emit_stop(self, active: dict[str, object], reason_code: str) -> SafetyDecision:
        command_port = active["command_port"]
        stop = getattr(command_port, "stop_body_velocity_and_clear", None)
        receipt = None
        if callable(stop):
            receipt = stop()
        clear_nav = getattr(command_port, "clear_pending_local_position_actions", None)
        if callable(clear_nav):
            clear_nav()
        decision = self._decision(active, reason_code)
        if receipt is None:
            return decision
        return replace(
            decision,
            cancellation_id=getattr(receipt, "cancellation_id", None),
            barrier_disposition=getattr(
                getattr(receipt, "barrier_disposition", None), "value", None
            ),
            barrier_id=getattr(receipt, "barrier_id", None),
        )
...
class ActionSafetyPipeline:
    def __init__(
        ...
        *,
        ...
        self._allow_test_source = allow_test_source
    ...
    def evaluate(
        ...
        *,
        ...
        # link_manager remains a test-only compatibility keyword while production
        # wiring supplies the explicit read-only state_port.
        state_port = state_port if state_port is not None else link_manager
        now = self._monotonic()
        ...
        if source == "test" and not self._allow_test_source:
        ...
        reason = self._telemetry_reason(action_type, source, state_port)
    ...
    def _telemetry_reason(self, action_type: str, source: str, manager: object | None) -> str | None:
        if action_type == "yolo_lock_target":
            return None
        if source == "test" and self._allow_test_source:
            return None
        if manager is None:
    ...
    def _field_reason(self, request: dict[str, object], *, global_target: bool) -> str | None:
        ...
        if not global_target:
            return "field_local_target_not_supported"
        ...
        if request.get("field_gps_transform_ready") is not True:
