    ...
    *,
    ...
    if frame == LOCAL_NED:
        ...
        if callable(wrapper):
            ...
            receipt = wrapper(x_north_m=x, y_east_m=y, z_down_m=z, yaw_rad=yaw, priority=priority)
            return (submission_outcome(receipt, _build_detail(action, x, y, z, frame, yaw)), log_msg)
    ...
    receipt = sender(x, y, z, frame, yaw=yaw, priority=priority)
    return (submission_outcome(receipt, _build_detail(action, x, y, z, frame, yaw)), log_msg)
...
def _fmt_goto_log(action: dict[str, object], yaw: float | None, priority: int) -> str:
    return (
        ...
        % (
            ...
            action.get("local_target"), yaw, priority, action.get("key"),
        )
    )
...
def _fmt_local_log(
    ...
    return (
        ...
        % (
            action.get("input_frame"), action.get("input_target"),
            action.get("local_target"), frame, yaw, priority, action.get("key"),
        )
    )
