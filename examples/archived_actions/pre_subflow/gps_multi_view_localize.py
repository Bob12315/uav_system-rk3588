...
from field.target_resolver import (
...
from guidance.target_projection import (
    GpsProjectionCamera,
    GpsProjectionError,
    GpsRawEstimate,
    GpsTargetProjector,
)
from guidance.gps_derived_enu_fusion import (
    GpsDerivedEnuFusion,
    GpsFusionConfig,
    GpsLocalizedObject,
)
...
class GpsMultiViewLocalizeAction(ActionModule):
    ...
    def _update_goto(self, context: dict[str, Any]) -> ActionResult:
        ...
        if not result.done:
            return ActionResult(effects=ActionResult.typed(result.actions), reason="gps_multi_view_goto", detail=self._detail(extra={"goto": result.detail}))

        # Goto done → settle directly (yaw is handled by field_heading goto)
        self.phase = "settle"
        self.settle_count = 0
    ...
    def _update_capture(self, context: dict[str, Any]) -> ActionResult:
        ...
        detections, img_w, img_h = self._detections(context)

        # ── filter: class + confidence ──
        target_classes = set(self._params_class_names) if self._params_class_names else None
        min_conf = self._params_min_confidence
        ...
        for det in keep:
            ex, ey = self._detection_ex_ey(det, img_w, img_h)
