...
class GpsTargetSequenceCore:
    def update(self, context: dict[str, Any] | None = None) -> ActionResult:
        ...
        if self.stopped: return ActionResult(effects=ActionResult.typed(self._stop_actions("1")), done=True, reason="stopped", detail=self._sequence_detail(done=True))
    ...
    def _update_goto(self, ctx):
        ...
        if not r.done:return ActionResult(effects=ActionResult.typed(r.actions),reason=self._sequence_reason('goto'),detail=self._sequence_detail())
    ...
    def _update_lock(self, ctx):
        ...
        if not result.failed:
            if not result.done:
                return ActionResult(
                    effects=ActionResult.typed(result.actions),
    ...
    def _update_align(self,ctx):
        ...
        if not r.done and isinstance(command,dict) and (not command.get('valid') or not command.get('active')): return ActionResult(effects=ActionResult.typed(self._stop_actions('align_inactive')),reason=self._sequence_reason('align_inactive'),detail=self._sequence_detail(extra={'align':r.detail}))
        if not r.done and command is None: return ActionResult(effects=ActionResult.typed([]),reason=self._sequence_reason('align'),detail=self._sequence_detail(extra={'align':r.detail}))
        if not r.done:return ActionResult(effects=ActionResult.typed([{'action_type':'flight_command','params':dict(command),'once':False,'key':self._action_key('align'),'priority':5}]),reason=self._sequence_reason('align'),detail=self._sequence_detail(extra={'align':r.detail}))
    ...
    def _update_climb(self,ctx):
        ...
        altitude=self._current_altitude_m(ctx)
        if altitude is not None and altitude >= self.climb_after_drop_m-self.climb_tolerance_z_m:return self._complete_climb()
        return ActionResult(effects=ActionResult.typed(r.actions),reason=self._sequence_reason('climb'),detail=self._sequence_detail(extra={'altitude_m':altitude}))
    def _complete_climb(self):
        ...
        self.phase='done'; return ActionResult(effects=ActionResult.typed(self._stop_actions('climb_done')),done=True,reason=self._sequence_reason('done'),detail=self._sequence_detail(done=True))
    def _transition(self,phase,event,actions=None): self.phase=phase; self.phase_history.append(phase); self.sub_action=None; self.update_count_at_phase=0; return ActionResult(effects=ActionResult.typed(actions or []),reason=self._sequence_reason(event),detail=self._sequence_detail())
    def _fail(self,event,actions=None,reason=None): self.phase='failed'; self._failed_reason=reason or self._sequence_reason(event); self.sub_action=None; return ActionResult(effects=ActionResult.typed(actions if actions is not None else self._stop_actions('failed')),failed=True,reason=self._failed_reason,detail=self._sequence_detail())
