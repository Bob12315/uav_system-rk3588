...
class MultiViewLocalizeAction(ActionModule):
    ...
    def start(self, params: dict[str, Any] | None = None) -> None:
        ...
        default_target_frame = "global" if self.waypoint_mode == "field" else "local"
        self.target_frame = str(data.get("target_frame", default_target_frame)).strip().lower()
        if self.target_frame not in {"local", "global"}:
            raise ValueError("target_frame must be local or global")
        if self.waypoint_mode == "field" and self.target_frame != "global":
            ...
            return ActionResult(
                ...
                detail=self._detail(extra={
                    ...
                    **{name: result.detail.get(name) for name in (
                        "input_frame", "input_target", "global_target", "field_heading_yaw_rad",
                    )},
                }),
            )
        if not result.done:
