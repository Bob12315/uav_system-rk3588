...
class GpsReconAreaScanAction(ActionModule):
    ...
    def update(self, context: dict[str, Any] | None = None) -> ActionResult:
        ...
        if not result.done:
            return ActionResult(effects=ActionResult.typed(result.actions), reason="gps_recon_area_scan_goto", detail=self._detail(extra={"goto": result.detail, "goto_reason": result.reason}))
