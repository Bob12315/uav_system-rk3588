...
class ReconInspectTargetAction(ActionModule):
    ...
    def update(self, context: dict[str, Any] | None = None) -> ActionResult:
        ...
        if self.state in {"goto_target", "target_lock", "align_descend"}:
            ...
            return ActionResult(effects=ActionResult.typed(list(result.actions)), reason=result.reason, detail=self._detail(result.detail))
    ...
    def _observe(self, context: dict[str, Any]) -> ActionResult:
        ...
        if self.observe_count < self.observe_updates:
            return ActionResult(effects=ActionResult.typed([zero]), reason="observe", detail=self._detail())
    ...
    def _finish(self, status: str, reason: str, actions: list[Any] | None = None) -> ActionResult:
        ...
        self.last_result = ActionResult(effects=ActionResult.typed(actions or []), done=True, reason=reason, detail=self._detail())
        return self.last_result
