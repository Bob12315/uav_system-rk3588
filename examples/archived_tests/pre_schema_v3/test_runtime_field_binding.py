"""Archived implementation-coupled runtime sampler tests."""
...
from field.profile import FieldProfile, parse_field_profile
from field.models import shortest_longitude_delta_deg
...
from field.geometry import RuntimeFieldGeometry
...
class TestSpreadAndGeometryFailure:
    ...
    def test_geometry_failure_sets_failed(self):
        ...
        import math as _m
        from field.models import EARTH_RADIUS_M as _ER
...
def test_no_local():
    src = Path("field/calibration.py").read_text()
    for token in (
        "local_x",
        "local_y",
        "local_z",
...
def test_no_clock():
    ...
    for token in ["import time", "from time ", "time.time", "time.monotonic", "sleep(",
                  ...
                  "FieldService", "SystemRunner", "LinkManager", "MAVLink"]:
        assert token not in src, f"forbidden: {token}"
