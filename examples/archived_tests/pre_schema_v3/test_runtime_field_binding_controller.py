"""Archived Field Controller/Service/Binding Orchestrator tests."""
...
from field.profile import parse_field_profile, validate_field_profile
from field.models import FieldReference
from field.reference_service import FieldReferenceService
from field.binding_orchestrator import RuntimeBindingOrchestrator, _synced
...
def test_no_local_in_orchestrator():
    src = __import__("pathlib").Path("field/binding_orchestrator.py").read_text()
...
def _controller_with_profile(monkeypatch):
    ...
    controller = FieldService(builder, lambda: {}, field_reference_service=svc)
...
def test_system_runner_preserves_mapping_snapshot():
    ...
    passed = runner.field_service.observe_runtime_profile_sampling.call_args.args[0]
...
def test_system_runner_converts_dataclass_snapshot():
    ...
    runner.field_service.observe_runtime_profile_sampling.assert_called_once_with(
...
def test_system_runner_isolates_controller_observe_exception():
    runner = _runner_sampling_spy()
    runner.field_service.observe_runtime_profile_sampling.side_effect = RuntimeError("boom")
...
def test_system_runner_observe_holds_action_runtime_lock():
    ...
    runner.field_service.observe_runtime_profile_sampling.side_effect = (
        assert_locked
    )
    runner._observe_runtime_field_sampling({}, now_s=10.0)
...
def test_system_runner_field_reference_handlers_hold_action_runtime_lock(
    ...
    getattr(runner.field_service, controller_method).side_effect = (
        assert_locked
    )
...
def test_system_runner_runtime_wrappers_use_controller_and_lock(
    ...
    runner.action_runtime_lock = threading.RLock()
    runner.field_service = Mock()
    getattr(runner.field_service, controller_method).return_value = {"ok": True}
    monkeypatch.setattr("application.runner.time.time", lambda: 123.0)
    args = ("test_orch",) if wrapper.endswith("start") else ()
    assert getattr(runner, wrapper)(*args) == {"ok": True}
    getattr(runner.field_service, controller_method).assert_called_once()
