...
def test_drop_v2_failure_and_stop_contract_is_covered_by_phase_suite() -> None:
    ...
    text = (
        (ROOT / "missions/common/actions/gps_target_sequence_core.py").read_text(encoding="utf-8")
        + (ROOT / "missions/common/actions/gps_drop_sequence.py").read_text(encoding="utf-8")
    )
    for required in (
    ...
    assert "test_climb_altitude_gate_wins_timeout_boundary" in (
        ROOT / "tests/unit/mission/test_feature3_gps_drop_control.py"
