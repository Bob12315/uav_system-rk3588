...
def test_field_waypoint_converts_to_global_target() -> None:
    action = MultiViewLocalizeAction()
    action.start(_params(
    ...
    result = action.update({
        "drone": {"global_position_valid": False, "lat": 34.0, "lon": 108.0, "relative_altitude": 3.0},
        "field_heading_yaw_rad": 0.0, "field_heading_confirmed": True,
        "field_origin_lat": 34.0, "field_origin_lon": 108.0,
        "field_origin_gps_confirmed": True,
    })

    assert result.detail["input_frame"] == "field"
    assert result.detail["global_target"]["lat"] > 34.0
    assert result.actions[0]["action_type"] == "global_goto"
