...
from missions.engine import MissionActionStep, MissionBlackboard, MissionOrchestrator
...
from tests.unit.mission import test_feature3_gps_drop_control as feature3
...
def _set_gps_reference(
    ...
    *,
    ...
    reference = runner.field_service.reference
...
def test_gps_mission_uses_one_shared_field_reference() -> None:
    runner = _make_runner()
    runner.configure_action_mission(_v2_steps())
    _set_gps_reference(runner)
    assert runner.runtime_context_builder.field_reference is runner.field_service.reference
...
def test_gps_only_mission_starts_without_local_origin() -> None:
    ...
    assert runner._action_mission_field_requirements() == {"needs_gps": True}
    payload = runner.action_mission_start(authorize=True, target_source="sitl")
...
def test_schema_v3_rejects_local_field_target_at_action_contract() -> None:
    from missions.common.actions.goto_waypoint import GotoWaypointAction
    with pytest.raises(ValueError, match="target_frame=global"):
