...
def test_waypoint_mode_absolute() -> None:
    ...
    paths = [
        repo_root / "examples" / "archived_missions" / "recon_sequence_v1.json",
        repo_root
        / "examples"
        / "archived_missions"
        / "sitl"
    ...
    return _FakeAction(
        [
            ActionResult(
                ...
                reason="sign_detected",
                effects=ActionResult.typed([zero_cmd]),
                detail={
                    "status": "detected",
                    "content": "baozha",
                    "confidence": 0.72,
...
def test_report_items_include_blank_and_skipped_targets() -> None:
    ...
    all_goto = [
        ...
        _FakeAction([ActionResult(reason="goto_active", effects=ActionResult.typed([]))]),  # t2: timeout
    ]
    lock_results = [
        _FakeAction([ActionResult(failed=True, reason="target_lock_timeout")]),
        _lock_done_factory(targets[1]),
...
def _observe_active_nonzero_factory() -> _FakeAction:
    ...
    return _FakeAction(
        [
            ActionResult(
                reason="observe_active",
                effects=ActionResult.typed([nonzero_cmd]),
                detail={
                    "status": "blank_or_uncertain",
                    "content": "blank",
                    "confidence": 0.0,
