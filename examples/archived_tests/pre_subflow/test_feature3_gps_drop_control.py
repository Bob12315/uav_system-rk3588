...
def test_climb_timeout_fails_no_next_target(children):
    ...
    class ScriptedGotoClimb:
        ...
        def update(self, c):
            ...
            key = self.p.get("key", "")
            if "climb" in key:
                return ActionResult(done=False, effects=ActionResult.typed([]))
...
def test_v2_align_config_matches_v1_except_payload_offset() -> None:
    ...
    import json
    v1 = json.loads(open("examples/archived_missions/drop_two_targets_v1.json").read())
...
class ScriptedGotoV2:
    ...
    def update(self, c):
        self.calls += 1
        return ActionResult(effects=ActionResult.typed([{"action_type": "global_goto", "params": {}, "once": False}])) if self.calls == 1 else ActionResult(done=True)
