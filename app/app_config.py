...
@dataclass(slots=True)
class ActionRuntimeDebugConfig:
...
class AppConfig:
    ...
    debug: ActionRuntimeDebugConfig
    start_gimbal: bool = False
    start_body: bool = False
    start_approach: bool = False
...
def _load_yolo_command_config(path: str) -> YoloCommandConfig:
    data = yaml.safe_load(Path(path).read_text(encoding="utf-8")) or {}
    command = data.get("command_receiver", {}) if isinstance(data, dict) else {}
    if not isinstance(command, dict):
        command = {}
    return YoloCommandConfig(
        ip=str(command.get("ip", "127.0.0.1")),
        port=int(command.get("port", 5006)),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--connect-telemetry", action="store_true")
...
def load_app_config(args: argparse.Namespace) -> AppConfig:
    ...
    return AppConfig(
        runtime=AppRuntimeConfig(
            yolo_udp_ip=args.yolo_udp_ip or str(runtime.get("yolo_udp_ip", "0.0.0.0")),
            ...
            connect_telemetry=bool(args.connect_telemetry or _bool(services, "connect_telemetry", False)),
        ...
        health=HealthMonitorConfig(), telemetry=telemetry, yolo_command=_load_yolo_command_config(args.yolo_config),
        ...
        debug=ActionRuntimeDebugConfig(), start_send_commands=send_commands,
    )
